function [class_cell,class_cell_num]=general_model_classication_2022_08_06_v1(varargin);
% This function classifies the experimental data JLB263. This is a cleanup
% verison
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
'\\slcu.cam.ac.uk\Data\Microscopy\TeamJL\Chris\movies\oscillations\2022-01-19\subAuto\Data\',...
'\\slcu.cam.ac.uk\Data\Microscopy\TeamJL\Chris\movies\oscillations\2022-01-26\subAuto\Data\'};
plot_now=2; %decids which plot to do: 1 plots MY; 2 plots the autocorrelation and all else plots the frequencey analysis
do_plot=0; %Do Quality control plots 0 no plot. 1 plots
plot_color=1; %setting colour for quality control plots as in on_colors_all
do_auto=0; %do autocorrelation for classification: 0 no autocorrelation 1 autcorrelation
L=576; %number of frames anlaysed

lag=36; %discarding the first 6h
name={'0 uM IPTG','2uM IPTG','3uM IPTG','4uM IPTG','5 uM IPTG','6 uM IPTG','7 uM IPTG','8 uM IPTG','9 uM IPTG'};%name of conditions
iptg=[0,2,3,4,5,6,7,8,9];%conditions in numbers
on_colors_all={'w','y','g','c'};%colors to plot
class_cell=zeros(4,length(iptg),length(data_path)); %setting output
class_cell_num={};%setting output


%%%%%%%%%%%%%%
%Checking for variable intput
%%%%%%%%%%%%
for i=1:2:length(varargin)
    ...
    elseif strcmp(varargin{i},'data_path')
...
for tt=1:length(data_path) %loop over all input data
    kk=0;
    for i=1:length(iptg) %loop over all iptg concentrations
        kk=kk+1;
        ...
        if ~isempty(D)
            ...
            for trace=1:do_now
                ...
                elseif sum(loc2>1)>0 && sum(freq(loc2)>10^-3)>0 %oscillation case
